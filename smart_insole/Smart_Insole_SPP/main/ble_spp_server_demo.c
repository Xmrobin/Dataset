...
#define GATTS_FSR_TAG    "GATTS_FSR_TAG"
...
void spp_cmd_task(void * arg)
{
    ...
    for(;;){
        ...
        vTaskDelay(50 / portTICK_PERIOD_MS);
        // if(xQueueReceive(cmd_cmd_queue, ble_data, portMAX_DELAY)) {
            if(is_connected){            
                uint16_t tx_size = strlen(ble_data);
                ESP_LOGI(GATTS_TABLE_TAG,"字节数=%d 字符串的长度=%d",sizeof(ble_data),tx_size);
                if(tx_size <= (spp_mtu_size-3)){
                	//ESP_LOGI(GATTS_TABLE_TAG,"%s",ble_data);
                    esp_ble_gatts_send_indicate(spp_gatts_if, spp_conn_id, spp_handle_table[SPP_IDX_SPP_DATA_NTY_VAL],tx_size, (uint8_t *)ble_data, false);
                }
            }          
        // }
    }
...
{
    spp_uart_init();
    cmd_cmd_queue = xQueueCreate(10, sizeof(char)*50);
...
void app_main(void)
{
    ...
    /* 初始化ADC */
    Init_ADC();
    xTaskCreate(ADC__task,"adc_Task",4096,NULL,2,NULL);

    /* 创建个任务处理需要蓝牙传输的数据 */
    //xTaskCreate(BLE_data_task,"BLE_data_task",2048,NULL,3,NULL);
...
static void ADC__task(void *arg)
{
    ...
    for (int i = 0; i < NO_OF_SAMPLES; i++) 
        ...
        //ESP_LOGI(GATTS_FSR_TAG,"%d-%d-%d-%d-%d-%d-%d-%d\n",adc_reading_0,adc_reading_3,adc_reading_4,adc_reading_5,adc_reading_6,adc_reading_7,adc2_reading_5,adc2_reading_4);
        sprintf(ble_data,"%d-%d-%d-%d-%d-%d-%d-%d %d %d %d\n",adc_reading_0,adc_reading_3,adc_reading_4,adc_reading_5,adc_reading_6,adc_reading_7,adc2_reading_5,adc2_reading_4,pitch_ble,roll_ble,yaw_ble);
    }
    vTaskDelete(NULL);
}
/* MPU6050 相关GPIO函数配置 */
...
// 		sprintf(ble_data,"%d-%d-%d-%d-%d-%d-%d-%d\n",adc_reading_0,adc_reading_3,adc_reading_4,adc_reading_5,adc_reading_6,adc_reading_7,adc2_reading_5,adc2_reading_4);
//         xQueueSend(cmd_cmd_queue,ble_data,100/portTICK_PERIOD_MS);
// 		ESP_LOGI(GATTS_TABLE_TAG,"%s",ble_data);
//     }
